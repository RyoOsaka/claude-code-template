...
6. 設計担当: スキーマ + モック作成 → 完了報告
   ↓
7. リーダー: バックエンド担当・フロントエンド担当を worktree でスポーン
   ↓
8. 並列実行（各メンバーは独立した worktree で作業）:
   - バックエンド担当: API 実装 + テスト
   - フロントエンド担当: 画面実装（モック使用）+ E2E テスト
   ↓
9. 統合: 各 worktree のブランチをマージ → コンフリクト解消
   ↓
10. フロントを実 API に接続 → E2E テスト実行
   ↓
...
各メンバーを `isolation: worktree` で起動し、独立した作業ディレクトリで作業する。

```
メインディレクトリ（リーダー）
├── .claude/worktrees/design/    （設計担当の worktree）
├── .claude/worktrees/backend/   （バックエンド担当の worktree）
└── .claude/worktrees/frontend/  （フロントエンド担当の worktree）
```

- 各メンバーが物理的に別ディレクトリで作業するため、同じファイルを編集しても競合しない
- 統合フェーズで Git マージにより変更を統合する
- 変更がない worktree は自動クリーンアップされる

### フォールバック: ディレクトリ分離ルール

worktree を使わない場合は、各担当が触るファイルをルールで分離する:
...
- 統合フェーズで Git マージ時にコンフリクトが発生する可能性がある
- コンフリクト解消はリーダーが調整する
- 変更がない worktree は自動クリーンアップされる
- 変更がある worktree はブランチとパスが返却されるので、マージ前に内容を確認する
